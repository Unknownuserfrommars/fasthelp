...
class _GetVersionInfo:
    def __init__(self):
        self._versionInfoMessage = "You are using FastHelp version 1.0.10, developed by JDT."
    def print(self):
        print(self._versionInfoMessage)
    def ret(self):
        return self._versionInfoMessage
    def pprint(self):
        pp.pprint(self._versionInfoMessage)
...
class DataHelper:
    ...
    class totype:
        def DataFrame(self, dataset):
            """
            You are using `DataHelper.totype.DataFrame()`, developed by JDT, version 0.0.4.
            ...
            if not isinstance(dataset, (pd.DataFrame, pd.Series)):
                ...
                raise TypeError(f'The dataset you\'ve entered is a {type(dataset)} type. Which is not okay.')
        ...
        def dict(self, dataset):
            """
            You are using `DataHelper.totype.dict()`, developed by JDT, version 0.1.2.
        ...
        def Array(self, dataset):
            """
            You are using `DataHelper.totype.Array()`, developed by JDT, version 0.3.1.
...
class future:
    class About:
        def __init__(self):
            self._about_future = "the `future` module contains many future implementations."
        def print(self):
            print(self._about_future)
        def ret(self):
            return self._about_future
        def pprint(self):
            pp.pprint(self._about_future)

    class Full_Read_Csv:
        def __init__(self, **kwargs):
            self.FirstAppearVersion = "1.0.9"
            self.FirstImplementVersion = "1.2.1"  # Hopefully...
            return pd.read_csv(**kwargs)
        
    class Full_Read_Excel:
        def __init__(self, **kwargs):
            self.FirstAppearVersion = "1.0.10"
            self.FirstImplementVersion = "1.2.1"
    ...
    class No_More_JDT:
        def __init__(self):
            ...
            self.FirstImplementVersion = "123456789.123456789.123456789"
...
class FileHelper:
    class Open:
        ...
        def Openexcel(self, file, sheet_name, index_col=None):
            """
            You are using `FileHelper.Open.Openexcel()`, developed by JDT, version 0.5.10.
