...
import typing
import packageimporter as pi # type: ignore  # noqa: F401
import sys
...
class ImportHelper:
    """
    Just copied src from original packageimporter.py with few mods and additions. NOTE THAT THESE ADDITIONS WILL NOT (AT LEAST NOT NOW) APPEAR IN THE ORIGINAL PACKAGEIMPORTER PACKAGE.
    """
    pyv = (3,10,0)
    pkgv = '1.4.1'
    ...
    def _check_python_version():
        if sys.version_info.major == 3 and sys.version_info.minor <= 9:
            raise ConnectionError(f"This program requires Python >= {'.'.join(map(str, ImportHelper.pyv))}, but you're using an incompatible version.")
        
    class PackageImporterVersionCheck:
        @staticmethod
        def Out():
            ImportHelper._check_python_version()
            print(f'Current Version: {ImportHelper.pkgv}')

        @staticmethod
        def Ret():
            ImportHelper._check_python_version()
            curr_ver = f'Current Version: {ImportHelper.pkgv}'
            return curr_ver
    ...
    class Pip:
        ...
        class Install:
            @staticmethod
            def __init__(package_name:str):
                ImportHelper._check_python_version()
                subprocess.check_call(sys.executable, "-m", "pip", "install", package_name)


    class Importer:
        def importo(pkg=None, alias=None):
            if pkg is None:
                raise ValueError("Package name must be provided")
            module = impl.import_module(pkg)
            if alias:
        ...
        class MyOwnModules:
            modules = {
                'pyprogramsimp': 'pprogsimp',
                'graphgame': 'graphg',
                'packageimporter': 'pkgimprt',
            }
            ...
            @classmethod
            def pyprogramsimp(cls, alias:str):
                ImportHelper._check_python_version()
                cls.import_module('pyprogramsimp', alias)

            @classmethod
            def graphgame(cls, alias:str):
                ImportHelper._check_python_version()
                cls.import_module('graphgame', alias)

            @classmethod
            def packageimporter(cls, alias:str):
                ImportHelper._check_python_version()
        ...
        class ScienceAndMaths:
            ...
            @staticmethod
            def import_module(module_name, alias:str=None):
                """
                Import a single module with optional alias, error is raised if package is not globally installed
                """
                ImportHelper._check_python_version()
                try:
                    ...
                    raise e(f"The module '{module_name}'is not globally installed, please install it first using pip.")
                ...
                def import_module(module_name, alias):
                    ...
                    try:
                        ...
                        if alias:
                            ...
                            default_alias = ImportHelper.Importer.Plotter.matplotlib.modules.get(module_name, module_name.split('.')[-1])
                ...
                def import_module(module_name, alias=None):
                    ...
                    try:
                        ...
                        if alias:
                            ...
                            default_alias = ImportHelper.Importer.Plotter.plotly.modules.get(module_name, module_name.split('.')[-1])
                            globals()[default_alias] = module
                    except ModuleNotFoundError as e:
                        raise e(f"The module '{module_name}'is not globally installed, please install it first using pip.)")
        ...
        class Builtins:
            ...
            def import_module(module_name, alias=None):
                ...
                try:
                    ...
                    if alias:
                        ...
                        default_alias = ImportHelper.Importer.Builtins.modules.get(module_name, module_name.split('.')[-1])
                        globals()[default_alias] = module
                except ModuleNotFoundError as e:
            ...
            @classmethod
            def math(cls, alias:str=None):
                ImportHelper._check_python_version()
                cls.import_module('math', alias)

class PlotHelper:
    def __init__(self):
        warnings.warn("This is not implemented yet. We WILL do it in version 1.2.0, where we add CLI and subpackages.")