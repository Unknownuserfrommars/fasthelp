...
class ImportHelper:
    """
    Just copied src from original packageimporter.py with few mods.
    """
    rv = (3,8,0)
    cv = '1.4.0'
    @staticmethod
    def _check_python_version():
        if sys.version_info.major == 3 and sys.version_info.minor <= 7:
            raise ConnectionError(f"This program requires Python >= {'.'.join(map(str, ImportHelper.rv))}, but you're using an incompatible version. Actually, the minumal version requirment for fasthelp is 3.10!")
        elif sys.version_info.major == 2 and sys.version_info.minor == 7 and sys.version_info.micro == 18:
            warnings.warn("You're using python 2.7.18, it's actually supported by the real code (WHAT!) So, it's an easter egg, i think.", FutureWarning)
        
    class PackageImporterVersionCheck:
        @staticmethod
        def Out():
            ImportHelper._check_python_version()
            print(f'Current Version: {ImportHelper.cv}')

        @staticmethod
        def Ret():
            ImportHelper._check_python_version()
            f = f'Current Version: {ImportHelper.cv}'
            return f
